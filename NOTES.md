...
- `_gen_meta_assets.py` запустить — **сначала добавить путь нового `.cs` в `BEHAVIOURS` set в начале скрипта**, иначе Pass 2 (`.asset`) и Pass 3 (editor stub) пропустят файл. Только `.cs.meta` создастся автоматически.
