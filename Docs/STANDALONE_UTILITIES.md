...
## ProTV *(требует `PSS_PROTV_INSTALLED`)*

### PSS_ProTVAmbientFade

**Спавн:** `Tools > PSS > Spawn > ProTV > ProTV Ambient Fade`

ProTV plugin — при старте воспроизведения запоминает текущие громкости указанных AudioSource и плавно глушит их до 0. При остановке воспроизведения восстанавливает оригинальные громкости с фейдом. Оригинальные громкости захватываются один раз при `Start()` — независимо от того, что происходило с громкостью после этого.

Подключается к ProTV через стандартный `TVPlugin` — нужно положить на объект в иерархии под `TVManager`, либо в дочерний объект.

**Поля:**

| Поле | Тип | По умолчанию | Описание |
|------|-----|-------------|----------|
| `sources` | `AudioSource[]` | — | Источники ambient звука для управления громкостью |
| `fadeDownDuration` | `float` | `1.5` | Секунды фейда 0 при старте воспроизведения |
| `fadeUpDuration` | `float` | `1.5` | Секунды восстановления громкости при остановке |

**Топология при спавне из меню:**
```
PSS_ProTVAmbientFade
└── PSS_ProTVAmbientFade (component)
    (положить в иерархию TVManager; sources — назначить вручную)
```

**Когда использовать:**
- Ambient-музыка или атмосферные звуки должны уступать место видео/стриму из ProTV
- Автоматическое dimming без дополнительных Udon скриптов
