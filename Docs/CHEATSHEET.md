...
### 1. Установка через VCC (рекомендуется)
Добавь репозиторий в VCC:
`vcc://vpm/add-repo?url=https://Pururut114.github.io/puru-signals-system/index.json`

Затем: **Projects → твой проект → Add Package → Puru Signals System**

Требования: **VRChat SDK3** + **UdonSharp** уже установлены в проекте.
После добавления пакет готов к использованию — дополнительный setup не нужен.

Если после установки UdonSharp жалуется на отсутствующие program assets — запусти **Tools → PSS → Repair Missing Program Assets**.
...
> **Требования для PSS_LtcgiControl:**
> 1. LTCGI установлен в проекте
> 2. `PSS_AutoSetup` обнаружит LTCGI при следующем domain reload и **автоматически** добавит `PSS_LTCGI_INSTALLED` + создаст program asset. Ручное добавление define не нужно.
> 3. Если автосетап не сработал — `Tools > PSS > Repair Missing Program Assets`
...
**Пример: LTCGI с late-join (GlobalState Bool — ручной сетап)**
```
...
```
Лейт-джоинер увидит актуальное состояние LTCGI автоматически через `OnDeserialization`.

> ⚠️ Wizard не поддерживает LTCGI Action — собирать этот сетап вручную.
...
### Телепорт игрока в точку (локальный)
```
[Button] PSS_OnInteract → ChannelLocal → PSS_TeleportPlayer (Target: SpawnPoint)
```

### Ресет позиции объекта
```
[Button] PSS_OnInteract → ChannelLocal → PSS_MoveToPoint (Object: Prop, Destination: SpawnPoint)
```

### Блокировка/разблокировка пикапов
```
[Enter Zone] PSS_OnEnterTrigger → ChannelLocal → PSS_SetPickupable (Targets: [prop1, prop2], Pickupable: false)
[Exit Zone]  PSS_OnExitTrigger  → ChannelLocal → PSS_SetPickupable (Targets: [prop1, prop2], Pickupable: true)
```

### Зона с изменением масштаба (точный рост)
```
[Enter] PSS_OnEnterTrigger → ChannelLocal → PSS_SetAvatarScale (SetSpecificHeight: true, EyeHeight: 0.5)
[Exit]  PSS_OnExitTrigger  → ChannelLocal → PSS_SetAvatarScale (SetSpecificHeight: false, AllowManual: true, Min: 0.2, Max: 5.0)
```
...
### PSS_TeleportPlayer
Телепортирует локального игрока к `target` Transform. `Y Offset` — смещение по вертикали (удобно для «посадить на стул» без точной настройки Transform-а).

**Пример: кнопка телепортирует в VIP-зону**
- `PSS_OnInteract` → `PSS_ChannelLocal` → `PSS_TeleportPlayer`
- Target: `VIP_SpawnPoint`, Y Offset: `0`
...
### PSS_MoveToPoint
Телепортирует Transform объекта в целевую точку. `Copy Rotation` — скопировать также ротацию. Подходит для ресета объекта, перемещения панели, мгновенного snap.

**Пример: кнопка сбрасывает пропс на исходное место**
- `PSS_OnInteract` → `PSS_ChannelLocal` → `PSS_MoveToPoint`
- Object to move: `Prop.transform`, Destination: `PropSpawnPoint`, Copy Rotation: ✓
...
### PSS_SetPickupable
Включает/выключает `VRC_Pickup.pickupable` на списке объектов. `Drop if disabling: true` — выбрасывает объект из рук перед блокировкой.

**Пример: выдать/убрать пикапы при входе в зону**
```
[Enter] PSS_OnEnterTrigger → ChannelLocal → PSS_SetPickupable (Items, Pickupable: true)
[Exit]  PSS_OnExitTrigger  → ChannelLocal → PSS_SetPickupable (Items, Pickupable: false, Drop: true)
```
...
### PSS_SetAvatarScale
Меняет рост локального игрока. Два режима:

| Режим | SetSpecificHeight | Что делает |
|-------|-------------------|-----------|
| Точный рост | `true` | `SetManualAvatarScalingAllowed(false)` + `SetAvatarEyeHeightByMeters` (0.1–100m) |
| Диапазон | `false` | `SetManualAvatarScalingAllowed(allow)` + min/max (0.2–5m) |

> Чтобы восстановить дефолт: `SetSpecificHeight: false`, `AllowManual: true`, `Min: 0.2`, `Max: 5.0`

**Пример: зона маленьких**
```
[Enter] PSS_OnEnterTrigger → ChannelLocal → PSS_SetAvatarScale (SetSpecific: true, Height: 0.4)
[Exit]  PSS_OnExitTrigger  → ChannelLocal → PSS_SetAvatarScale (SetSpecific: false, AllowManual: true)
```

---

## ProTV *(условная сборка — `PSS_PROTV_INSTALLED`)*

> **Требования для PSS_ProTVAccessGate:**
> 1. ProTV установлен в проекте
> 2. `PSS_AutoSetup` обнаружит ProTV при следующем domain reload и **автоматически** добавит `PSS_PROTV_INSTALLED` + создаст program asset. Ручное добавление define не нужно.
> 3. Если автосетап не сработал — `Tools > PSS > Repair Missing Program Assets`
>
> Компонент доступен в меню **Add Component → PSS/Standalone Utilities/ProTV**.

### PSS_ProTVAccessGate
Access gate на основе ProTV whitelist + TVManager. Проверяет `_IsAuthorized` / `_IsSuperAuthorized` + `_IsAuthorizedUser` / `_IsSuperUser` — логическое OR. Поддерживает пять независимых систем:

| Система | Что делает |
|---------|-----------|
| **Panel teleport** | Двигает `panelObject` между `unauthorizedPosition` и `authorizedPosition` |
| **Avatar scaling** | SetLimits (min/max) или SetExactHeight (точный рост) для авторизованных |
| **Objects gate** | `disableWhenAuthorized` / `enableWhenAuthorized` — SetActive по доступу |
| **Colliders gate** | `disableCollidersWhenAuthorized` / `enableCollidersWhenAuthorized` |
| **Pickups gate** | `restrictPickupsForNonAuthorized` — Drop + pickupable=false для неавторизованных |

Все системы независимы — неиспользуемые поля оставь пустыми.

**Типичный сетап:**
1. Добавь `PSS_ProTVAccessGate` на дочерний объект к `TVManager`
2. `whitelist` и `tvManager` подтянутся автоматически через `GetComponentInParent`
3. Назначь `unauthorizedPosition` (вне видимости) и `authorizedPosition` (в видимости)
4. Опционально: `enableWhenAuthorized` = `[AdminPanel]`, `disableWhenAuthorized` = `[GuestWall]`

---

## Standalone Utilities

Самодостаточные компоненты — без Trigger/Channel/Action. Добавляются через `Tools > PSS > Spawn > ...`.
...
Включает/выключает список объектов пока локальный игрок внутри trigger-зоны. `Invert` — объекты активны снаружи. Автосбор коллайдеров с себя и детей.

**Пример: ambient эффект только внутри зоны**
```
PSS_ZoneEnableWhileInside
  targets: [AmbientFX, AmbientSound]
  invert: false
...
Вход в зону → fade to black → телепорт → fade back. Защита от повторного срабатывания во время анимации.

**Пример: fall zone под картой**
```
PSS_FallZoneBlackoutTeleport
  postVolume: BlackoutVolume
  teleportTarget: SpawnPoint
  fadeInSeconds: 0.5, holdSeconds: 0.5, fadeOutSeconds: 0.5
```

---

#### PSS_ZoneReparentSnap
Вход в зону → reparent `targetRoot` к `enterMarker` + snap local transform. Опциональный `exitMarker` для обратного snap при выходе.

**Пример: UI панель прикрепляется к anchor при входе в комнату**
```
PSS_ZoneReparentSnap
  targetRoot: UIPanel
  enterMarker: RoomAnchor
  exitMarker: LobbyAnchor    (опционально)
  snapPosition/Rotation/Scale: true
```
...
#### PSS_FadeOnJoin
PostProcessVolume weight 1→0 при старте мира. `forceDarkFrames` защищает от flash при инициализации. Опциональный `holdSeconds` — держать черный пока сцена грузится.

**Пример: плавное появление мира**
```
PSS_FadeOnJoin
  volume: BlackoutVolume      (или GetComponent с того же объекта)
  forceDarkFrames: 3
  holdSeconds: 1.0
  fadeDuration: 2.0
...
**Пример: fade только для вернувшихся игроков (через PSS_PositionPersistence)**
```
PSS_PositionPersistence → onRestoredChannel → ChannelLocal → PSS_FadeOnJoin.Begin()
PSS_FadeOnJoin: autoStart = false
```
...
Interact на объекте → fade 0→1 → teleport → fade 1→0. `triggerOnce` отключает после первого использования. Fade опциональный — без volume работает как мгновенный телепорт.

**Пример: дверь-портал**
```
PSS_InteractTeleport
  destination: RoomB_SpawnPoint
  fadeVolume: BlackoutVolume
  fadeDuration: 0.5, holdSeconds: 0.2
  triggerOnce: false
```
...
#### PSS_PickupPortal
Пикап-пульт: нажал Use → fade → teleport игрока → fade back → respawn самого пикапа в `remoteRespawnPoint`. Rigidbody velocity сбрасывается.

**Пример: магический артефакт-телепортер**
```
PSS_PickupPortal         (на том же объекте что VRC_Pickup + VRCObjectSync)
  playerDestination: SecretRoom_Spawn
  remoteRespawnPoint: ArtifactPedestal
  requireHeld: true
  dropAfterUse: true
  fadeVolume: BlackoutVolume
```
...
#### PSS_MultiSelectController + PSS_MultiSelectButton
`MultiSelectController` — synced (Manual) radio-selector: ровно один из `targets[]` активен по индексу. -1 = все выключены. `SelectToggle` — повторный выбор снимает выбор.

`MultiSelectButton` — кнопка для контроллера. `toggleMode` — повторный клик деактивирует.

`onSelectChannels[]` — опциональный массив PSS_ChannelLocal: при выборе индекса N срабатывает канал N.

**Пример: рейв пульт с 3 пресетами эффектов**
```
PSS_MultiSelectController
  targets: [EffectsA_Root, EffectsB_Root, EffectsC_Root]
  defaultSelectedIndex: -1
  broadcastApplyForInstantFeedback: true
  onSelectChannels: [ChannelA, ChannelB, ChannelC]   (опционально)

Кнопка 1: PSS_MultiSelectButton → controller: Controller, indexToSelect: 0
Кнопка 2: PSS_MultiSelectButton → controller: Controller, indexToSelect: 1
Кнопка 3: PSS_MultiSelectButton → controller: Controller, indexToSelect: 2
Кнопка OFF: PSS_MultiSelectButton → indexToSelect: -1
```
...
Сохраняет/восстанавливает позицию и ротацию игрока между визитами через VRChat PlayerData API. `_restored` флаг предотвращает перезапись данных до загрузки persistence.

`useCheckpointsOnly: true` — авто-loop выключен, сохранения только по `OnCheckpointReached()`.

`onRestoredChannel` — срабатывает после восстановления позиции (только если данные есть, не в первый визит).

**Пример: большой открытый мир**
```
PSS_PositionPersistence
  keyPrefix: "PSS"
  saveInterval: 10
  useCheckpointsOnly: false
```

**Пример: checkpoint-based мир**
```
PSS_PositionPersistence
  useCheckpointsOnly: true

На каждом чекпоинте:
  PSS_OnEnterTrigger → ChannelLocal → PSS_CallMethod
    (Target: PositionPersistence, Event: "OnCheckpointReached")
```

---

## Быстрые рецепты — Standalone

### Fade-in при заходе в мир
```
PSS_FadeOnJoin (autoStart: true, holdSeconds: 1, fadeDuration: 2)
```

### Дверь через Interact с затемнением
```
PSS_InteractTeleport (destination, fadeVolume, fadeDuration: 0.5)
```

### Пикап-телепортер с возвратом
```
PSS_PickupPortal (playerDestination, remoteRespawnPoint, requireHeld: true)
```

### Радио-кнопки для пресетов (synced)
```
PSS_MultiSelectController (targets: [A, B, C], defaultSelectedIndex: -1)
PSS_MultiSelectButton × 3 (indexToSelect: 0/1/2)
```

### Тогл одного пресета кнопкой
```
PSS_MultiSelectButton (indexToSelect: 0, toggleMode: true)
```

### Сохранение позиции между визитами
```
PSS_PositionPersistence (saveInterval: 10)
```

### Объект снапается к anchor при входе в зону
```
PSS_ZoneReparentSnap (targetRoot, enterMarker, exitMarker)
```

### Включить объекты только внутри зоны
```
PSS_ZoneEnableWhileInside (targets: [FX], evaluateOnStart: true)
```

### Fade-in только для вернувшихся игроков
```
PSS_PositionPersistence (onRestoredChannel → ChannelLocal → PSS_FadeOnJoin.Begin())
PSS_FadeOnJoin (autoStart: false)
```
