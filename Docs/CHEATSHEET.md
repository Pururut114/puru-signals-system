...
> 2. В **Project Settings → Player → Scripting Define Symbols** добавлен `PSS_LTCGI_INSTALLED`
> 3. После этого — `Tools > PSS > Repair Missing Program Assets` (создаёт program asset для LtcgiControl)
...
> 2. В **Project Settings → Player → Scripting Define Symbols** добавлен `PSS_PROTV_INSTALLED`
> 3. После этого — `Tools > PSS > Repair Missing Program Assets` (создаёт program asset)
>
