...
namespace PuruSignals
{
    ...
    public class PSS_MasterVisibility : UdonSharpBehaviour
    {
        ...
        [Tooltip("Fires when local player becomes master (on Start or when previous master leaves)")]
        public PSS_ChannelLocal onMasterChannel;
        [Tooltip("Fires if local player is not master on Start")]
        public PSS_ChannelLocal onNonMasterChannel;
        ...
        private void Apply(bool isMaster)
        {
            ...
            if (isMaster && onMasterChannel != null) onMasterChannel.Trigger();
            if (!isMaster && onNonMasterChannel != null) onNonMasterChannel.Trigger();
        }
    }
}
