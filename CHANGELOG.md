...
### Added
- `PSS_ChannelGlobal.masterOnly` — bool флаг. Если включён, только Instance Master может отправлять глобальное событие. Решает проблему дублирования событий при подключении `OnTimer` / `OnSpawn` / `OnEnable` к Global каналу: без masterOnly каждый клиент генерировал независимый сетевой fire, что давало N срабатываний на N клиентов. Отображается в PSS_Node Network секции.
